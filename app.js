...
function renderProcessFlowSVG(steps, layerMap, groups, selectedStapNr) {
  ...
  return `<svg viewBox="0 0 ${svgWidth} ${svgHeight}" style="width:100%;max-width:${svgWidth}px;height:auto;display:block;" xmlns="http://www.w3.org/2000/svg">
    <defs>
...
const mcOpenFases = new Set(); // welke fases staan uitgeklapt (nrs) — leeg = alles dicht
  ...
  return `<div class="mc-toolbar">
    ...
    <div class="mc-toolbar-row2">
      ...
      <div class="mc-toolbar-actions">
        <button type="button" class="mc-toolbtn" data-mcall="open">Alles uitklappen</button>
        <button type="button" class="mc-toolbtn" data-mcall="close">Alles inklappen</button>
      </div>
    </div>
  </div>`;
}

/* Eén fase-kaart. Bij een actieve zoekterm: fases met een treffer
   klappen automatisch open (alleen de treffers gemarkeerd), fases
   zonder treffer dimmen. Zonder zoekterm geldt de open/dicht-status
   uit mcOpenFases. */
function mcFaseCardHtml(f, q) {
  const st = MC_CAT_STYLE[f.cat] || MC_CAT_STYLE.toegang;
  const matchedVragen = q ? f.vragen.filter(v => v.naam.toLowerCase().includes(q)) : f.vragen;
  const nameMatch = q && f.naam.toLowerCase().includes(q);
  const hasMatch = !q || matchedVragen.length > 0 || nameMatch;
  const open = q ? (matchedVragen.length > 0) : mcOpenFases.has(f.nr);
  const shownVragen = q ? matchedVragen : f.vragen;

  // Ingeklapt: compacte type-verdeling (aantal keuze/datum/getal) als
  // gekleurde mini-badges, zodat de kop ook dicht al informatie geeft.
  const counts = {};
  ...
  const tagsHtml = f.vragen.length
    ? `<div class="mc-tags">${shownVragen.map(v => mcVraagTagHtml(v, q)).join('')}</div>`
  ...
  return `
    <div class="mc-fase ${f.optioneel ? 'mc-fase-opt' : ''} ${q && !hasMatch ? 'mc-fase-dim' : ''}" id="mcfase-${f.nr}" data-mcfase="${f.nr}">
      <div class="mc-fase-tag" style="background:${st.header};">${escHtml(st.label.split(' ')[0])}</div>
      <div class="mc-fase-body">
        <div class="mc-fase-hdr" style="background:${st.header};">
          <div class="mc-fase-hdr-l">
            <span class="mc-fase-nr">${f.nr}</span>
            <span class="mc-fase-naam">${q ? hlHtml(f.naam, q) : escHtml(f.naam)}</span>
            ${f.optioneel ? `<span class="mc-fase-optbadge">optioneel</span>` : ''}
          </div>
          <div class="mc-fase-hdr-r">
            ${open ? '' : miniHtml}
            <span class="mc-fase-count">${q ? `${matchedVragen.length}/${f.vragen.length}` : f.vragen.length} vragen</span>
            <span class="mc-fase-toggle-icon">${open ? '▲' : '▼'}</span>
          </div>
        </div>
        ${open ? `<div class="mc-fase-data">${tagsHtml}</div>${vervolgHtml}` : ''}
      </div>
    </div>`;
}
...
function renderMcFlowHtml(proc) {
  ...
  sections.forEach((sec, si) => {
    if (si > 0) parts.push(`<div class="mc-conn"><div class="mc-arr"></div></div>`);

    if (sec.type === 'seq') {
      sec.fases.forEach((f, i) => {
        if (i > 0) parts.push(`<div class="mc-conn"><div class="mc-arr"></div></div>`);
        parts.push(mcFaseCardHtml(f, q));
      });
      return;
    }

    // Parallel blok: kolom per spoor. Bij één spoor komt er links een
    // "regulier pad"-doorloopkolom bij, zodat zichtbaar is dat het
    // hoofdpad gewoon doorloopt terwijl dit pad ernaast bestaat.
    const lanes = [...sec.lanes.entries()];
    const laneHtml = lanes.map(([label, fases]) => {
      const inner = fases.map((f, i) =>
        (i > 0 ? `<div class="mc-conn mc-conn-sm"><div class="mc-arr"></div></div>` : '') + mcFaseCardHtml(f, q)
      ).join('');
      return `<div class="mc-lane">
        <div class="mc-lane-label">${escHtml(label)}</div>
        ${inner}
      </div>`;
    });
    if (lanes.length === 1) {
      laneHtml.unshift(`<div class="mc-lane mc-lane-ghost">
        <div class="mc-lane-label">Regulier pad</div>
        <div class="mc-ghost-track"><div class="mc-arr"></div></div>
        <div class="mc-ghost-note">loopt direct door</div>
      </div>`);
    }
    parts.push(`<div class="mc-split">${laneHtml.join('')}</div>`);
  });

  return `<div class="mc-flow">${parts.join('')}</div>`;
}
...
function toggleMcFase(faseNr) {
  const p = procEntries.find(x => x.id === currentProcId);
  if (!p || p.type !== 'menscentraal') return;
  if (mcSearchQuery.trim()) return; // tijdens zoeken bepaalt de zoekterm wat open is
  if (mcOpenFases.has(faseNr)) mcOpenFases.delete(faseNr);
  else mcOpenFases.add(faseNr);
  renderMcFlow(p);
}

/* Springen vanuit de mini-navigatiebalk: fase openklappen + ernaartoe
   scrollen (scroll-margin-top op .mc-fase compenseert de sticky balk). */
function jumpToMcFase(faseNr) {
  const p = procEntries.find(x => x.id === currentProcId);
  if (!p || p.type !== 'menscentraal') return;
  mcOpenFases.add(faseNr);
  renderMcFlow(p);
  const target = document.getElementById('mcfase-' + faseNr);
  if (target) target.scrollIntoView({ behavior: 'smooth', block: 'start' });
}

function toggleAllMcFases(mode) {
  const p = procEntries.find(x => x.id === currentProcId);
  if (!p || p.type !== 'menscentraal') return;
  if (mode === 'open') p.fases.forEach(f => mcOpenFases.add(f.nr));
  else mcOpenFases.clear();
  renderMcFlow(p);
}
...
function selectProcEntry(id) {
  ...
  if (p.type === 'menscentraal') {
    mcOpenFases.clear(); // start altijd overzichtelijk: alle fases dicht
    mcSearchQuery = '';
    flowWrap.style.justifyContent = 'stretch';
    // Belangrijk: géén overflow-x:auto hier — een overflow-container
    // tussen de sticky toolbar en het scrollende detailpaneel breekt
    // position:sticky. Horizontaal scrollen gebeurt per .mc-split.
    flowWrap.style.overflowX = 'visible';
    ...
    sub.textContent = `${p.fases.length} zaakstappen · ${totalVragen} vragen — klik op een stap, zoek een vraag of spring via de balk`;
  } else {
    flowWrap.style.justifyContent = 'center';
    flowWrap.style.overflowX = 'auto'; // brede SVG-flows mogen wél horizontaal scrollen
    const layerMap = computeStepLayers(p.steps);
    const groups = groupByLayer(p.steps, layerMap);
    flowWrap.innerHTML = renderProcessFlowSVG(p.steps, layerMap, groups, null);
    legend.innerHTML = NORMAAL_LEGEND;
    sub.textContent = `${p.steps.length} stappen`;
  }
...
function selectProcStep(stapNr) {
  ...
  const layerMap = computeStepLayers(p.steps);
  const groups = groupByLayer(p.steps, layerMap);
  document.getElementById('procFlowWrap').innerHTML = renderProcessFlowSVG(p.steps, layerMap, groups, stapNr);
...
function initInteractions() {
  ...
  document.getElementById('procFlowWrap').addEventListener('click', ev => {
    ...
    if (mcJump) { jumpToMcFase(parseInt(mcJump.dataset.mcjump, 10)); return; }
    const mcAll = ev.target.closest('[data-mcall]');
    if (mcAll) { toggleAllMcFases(mcAll.dataset.mcall); return; }
    const mcFase = ev.target.closest('[data-mcfase]');
    if (mcFase) { toggleMcFase(parseInt(mcFase.dataset.mcfase, 10)); return; }
