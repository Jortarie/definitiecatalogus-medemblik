...
//  Alle inhoudelijke data (definities, processen, datavelden,
//  dashboards) wordt bij het opstarten opgehaald uit een publiek
//  gepubliceerd Google Sheet (CSV-export per tabblad). Lukt het
//  ophalen niet (geen internet, sheet nog niet gepubliceerd, etc.),
//  dan valt de app terug op de hardcoded DEFAULT_* datasets hierboven
//  zodat de app nooit leeg of kapot is.
// ═══════════════════════════════════════════════════════════════

// TODO: vul deze 4 URLs in met de "Publiceren op het web" CSV-links
// van elk tabblad in het Google Sheet (Bestand → Delen → Publiceren
// op het web → kies tabblad → formaat CSV → kopieer link).
// Processen blijft (voorlopig) hardcoded in DEFAULT_PROC — geen sheet.
const SHEET_URLS = {
  definities:  'https://docs.google.com/spreadsheets/d/e/2PACX-1vRqtLb8sAkrRePaCRjZSMVrqvC_lB3CEkUJ6S1MnfIFdzH-L7ABkSNAPuOaglIzR3mMVgABiAoBjIej/pub?gid=1384524365&single=true&output=csv',
  datavelden:  'https://docs.google.com/spreadsheets/d/e/2PACX-1vRqtLb8sAkrRePaCRjZSMVrqvC_lB3CEkUJ6S1MnfIFdzH-L7ABkSNAPuOaglIzR3mMVgABiAoBjIej/pub?gid=1363902042&single=true&output=csv',
  dashboards:  'https://docs.google.com/spreadsheets/d/e/2PACX-1vRqtLb8sAkrRePaCRjZSMVrqvC_lB3CEkUJ6S1MnfIFdzH-L7ABkSNAPuOaglIzR3mMVgABiAoBjIej/pub?gid=369067592&single=true&output=csv'
};

// ── Robuuste CSV-parser ───────────────────────────────────────────
// Ondersteunt: komma's en newlines binnen quotes, dubbele quotes als
// escape ("") en CRLF/LF. Geeft een array van objecten terug met de
// headerrij als keys.
function parseCSV(text) {
  ...
  // laatste veld/regel (bestand eindigt niet altijd met newline)
  ...
  for (let r = 1; r < rows.length; r++) {
    const rawRow = rows[r];
    // sla volledig leeg rijen over (kan ontstaan door trailing newline)
...
// Velden die in de CSV als JSON-tekst zijn opgeslagen (geneste
// structuren zoals fieldChips, nodes, edges) weer terugzetten naar
// echte arrays/objecten. Lege cel → fallback naar leeg array.
function parseJsonField(value, fallback) {
  if (value === undefined || value === null || value === '') return fallback;
  try { return JSON.parse(value); } catch (e) { return fallback; }
}

// Geneste structuren (fieldChips, changelog, nodes, edges) staan in
// de CSV als Base64-tekst — dit voorkomt dat Google Sheets struikelt
// over de vele aanhalingstekens in ruwe JSON. Hier weer terugzetten
// naar de echte array/object.
function unb64Field(value, fallback) {
  if (value === undefined || value === null || value === '') return fallback;
  try {
...
// Leesbare tekstvelden met interne regeleinden (fields, fieldsFull,
// notes, def, ctx, values, desc) staan in de CSV met een letterlijke
// "\n" in plaats van een echte regeleinde, zodat Google Sheets de cel
// niet als rij-einde interpreteert. Hier weer omzetten naar een echte
// newline voor weergave in de app.
function unescapeTextField(value) {
  if (value === undefined || value === null) return '';
  return String(value).split('\\n').join('\n');
}

// "true"/"false" tekst uit Sheets teruglezen als boolean
...
// Map de ruwe CSV-rijen (platte strings) terug naar de datastructuur
// die de rest van de app verwacht, per dataset.
function mapDefinitiesRows(rows) {
  return rows.map(r => ({
    id: r.id,
    term: r.term,
    def: unescapeTextField(r.def),
    ctx: unescapeTextField(r.ctx),
    fields: unescapeTextField(r.fields),
    cat: r.cat,
    team: r.team,
    status: r.status,
    date: r.date,
    notes: unescapeTextField(r.notes),
    fieldChips: unb64Field(r.fieldChips, []),
    fieldsFull: unescapeTextField(r.fieldsFull),
    source: r.source,
    changelog: unb64Field(r.changelog, [])
  }));
}

function mapDataveldenRows(rows) {
  return rows.map(r => ({
    id: r.id,
    name: r.name,
    desc: unescapeTextField(r.desc),
    type: r.type,
    src: r.src,
    col: r.col,
    values: unescapeTextField(r.values),
    ext: parseBoolField(r.ext)
  }));
}

function mapDashboardsRows(rows) {
  return rows.map(r => ({
    id: r.id,
    name: r.name,
    type: r.type,
...
// Haalt de 3 datasets (Definities, Datavelden, Dashboards) parallel
// op uit Google Sheets. Geeft per dataset óf de data uit het sheet,
// óf (bij falen/leeg) de hardcoded DEFAULT_* terug.
// Processen blijft voorlopig hardcoded (DEFAULT_PROC) — geen sheet.
async function loadAllFromSheets() {
  const [defRows, dvRows, dashRows] = await Promise.all([
    fetchSheetCSV(SHEET_URLS.definities),
    fetchSheetCSV(SHEET_URLS.datavelden),
    fetchSheetCSV(SHEET_URLS.dashboards)
  ]);

  entries    = (defRows  && defRows.length)  ? mapDefinitiesRows(defRows)  : JSON.parse(JSON.stringify(DEFAULT_DATA));
  processes  = JSON.parse(JSON.stringify(DEFAULT_PROC));
  dvEntries  = (dvRows   && dvRows.length)   ? mapDataveldenRows(dvRows)  : JSON.parse(JSON.stringify(DV_DEFAULT));
  dashboards = (dashRows && dashRows.length) ? mapDashboardsRows(dashRows): JSON.parse(JSON.stringify(DASH_DEFAULT));
}
...
function saveData() { /* no-op: data wordt centraal beheerd in Google Sheets */ }
...
function loadProcesses() {
  // Data wordt geladen door loadAllFromSheets(); dit is een fallback
  // voor het geval die nog niet is uitgevoerd.
...
function saveProcesses() { /* no-op: data wordt centraal beheerd in Google Sheets */ }
...
function loadDvData() {
  // Data wordt geladen door loadAllFromSheets(); dit is een fallback
  // voor het geval die nog niet is uitgevoerd.
  if (!dvEntries || !dvEntries.length) {
    dvEntries = JSON.parse(JSON.stringify(DV_DEFAULT));
  }
}

function saveDvData() { /* no-op: data wordt centraal beheerd in Google Sheets */ }
...
// ── Data verversen vanuit Google Sheets ──────────────────────────
// Vervangt de oude "Publiceer naar GitHub"-flow: data wordt niet meer
// in de HTML gebakken, maar leeft centraal in het Google Sheet. Deze
// knop haalt gewoon opnieuw de actuele data op (handig als iemand het
// sheet net heeft bijgewerkt en niet wil wachten op een herlaad).
async function refreshFromSheets() {
  ...
  try {
    ...
    rebuildDvFilterSrc();
    renderTable();
    if (typeof renderProcLibrary === 'function' && document.getElementById('view-processen').style.display !== 'none') renderProcLibrary();
    if (typeof renderDvTable === 'function') renderDvTable();
    if (typeof renderDashTable === 'function') renderDashTable();
    updateHeaderCounts();
    document.getElementById('ftrDate').textContent = 'Bijgewerkt op ' + new Date().toLocaleDateString('nl-NL', {day:'2-digit',month:'long',year:'numeric'}) + ' ' + new Date().toLocaleTimeString('nl-NL', {hour:'2-digit',minute:'2-digit'});
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = 'Vernieuw data'; }
  }
}

// Init — data wordt asynchroon opgehaald uit Google Sheets; tot die
// klaar is, draait de app op de hardcoded DEFAULT_* data zodat het
// scherm niet leeg is tijdens het laden.
loadMeta();
loadData();
loadProcesses();
loadDvData();
...
loadAllFromSheets().then(function() {
  rebuildDvFilterSrc();
  renderTable();
  updateHeaderCounts();
}).catch(function() {
  // fetch mislukt volledig — blijf op de hardcoded data die al geladen is
});
...
function saveDashboards() { /* no-op: data wordt centraal beheerd in Google Sheets */ }
function nextDashId() {
