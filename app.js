...
/* ════════════════════════════════════════════════════════════════
   TOEGANG JEUGD 3.0 – PROCESSCHEMA & ZAAKSTAP DATA
   Bron: Werkproces & Dataschema Sociaal Domein · Gemeente Medemblik
   ════════════════════════════════════════════════════════════════ */

const JEUGD_ZAAKSTAPPEN = [
  {
    id: 'stap-10',
    num: 'Zaakstap 10',
    title: 'Melding',
    sub: 'iJw § Toegang',
    color: '#005496',
    fase: 'Melding & Onderzoek',
    preview: ['Datum ontvangst melding','Woonplaatsbeginsel gecontroleerd','Triage uitkomst'],
    secties: [
      { titel: 'Kerngegevens Melding', velden: [
        { label: 'Datum ontvangst melding', type: 'g' },
        { label: 'Datum woonplaatsbeginsel gecontroleerd in CMG', type: 'g' },
        { label: 'Datum verhuizing naar gemeente Medemblik', type: 'g' },
        { label: 'Woonplaatsbeginsel: Is Medemblik de uitvoerende gemeente?', type: 'b' },
        { label: 'Is de jeugdige jonger dan 18 jaar?', type: 'b' },
        { label: 'Type melding', type: 'b' },
        { label: 'Aanmelding via', type: 'c' },
        { label: 'Is er toestemming van de gezaghebbenden?', type: 'b' },
        { label: 'Zijn de ouders op de hoogte van de aangevraagde zorg?', type: 'b' },
        { label: 'Van welke gemeente komt de overdracht?', type: 'c' },
      ]},
      { titel: 'Triage', velden: [
        { label: 'Uitkomst triage', type: 'b' },
        { label: 'Motivatie triage', type: 'c' },
        { label: 'Welke actie wordt er nu ondernomen?', type: 'b' },
      ]},
      { titel: 'Ouders / Verzorgers (herhaalbaar)', velden: [
        { label: 'Rol ouder/verzorger', type: 'c' },
        { label: 'Gezag', type: 'b' },
        { label: 'Naam, geboortejaar, adres, telefoon, e-mail (per ouder/verzorger)', type: 'c' },
      ]},
      { titel: 'Contactpersonen & Zorgnetwerk', velden: [
        { label: 'Naam school + contactpersoon school', type: 'c' },
        { label: 'Naam huisarts + praktijk', type: 'c' },
        { label: 'Naam medisch specialist + ziekenhuis', type: 'c' },
        { label: 'Koppeling vorige zaak + startdatum vorig arrangement', type: 'g' },
      ]},
    ],
    vervolgpaden: [
      { label: '→ Besluitvorming', desc: 'Directe besluitvorming mogelijk', type: 'ok' },
      { label: '→ Onderzoek', desc: 'Nader onderzoek noodzakelijk', type: 'opt' },
      { label: '→ Archiveren', desc: 'Geen verdere actie nodig', type: 'back' },
    ]
  },
  {
    id: 'stap-20',
    ...
    color: '#005496',
    fase: 'Melding & Onderzoek',
    preview: ['Datum gesprek','Hulpvraag omschrijving','Leefgebieden','Resultaten 1–6'],
    secties: [
      { titel: 'Gesprek(ken)', velden: [
        { label: 'Om welk Perspectiefplan gaat het?', type: 'b' },
        { label: 'Datum gesprek', type: 'g' },
        { label: 'Datum tussenevaluatie', type: 'g' },
        { label: 'Datum eindevaluatie', type: 'g' },
        { label: 'Locatie gesprek', type: 'c' },
        { label: 'Wie zijn er aanwezig? (herhaalbaar)', type: 'c' },
        { label: 'Vervolggesprekken: datum + aanwezigen', type: 'g' },
      ]},
      { titel: 'Hulpvraag & Voorliggend Veld', velden: [
        { label: 'Kan er hulp geboden worden uit het voorliggende veld?', type: 'b' },
        { label: 'Zijn er zorgen om de Veiligheid?', type: 'b' },
        { label: 'Hulpvraag (omschrijving)', type: 'c' },
        { label: 'Eerdere jeugdhulpondersteuning?', type: 'b' },
        { label: 'Motivatie gezin / jeugdige', type: 'c' },
      ]},
      { titel: 'Leefgebieden', velden: [
        { label: 'Functioneren en ontwikkeling', type: 'c' },
        { label: 'Sociale contacten', type: 'c' },
        { label: 'Gezondheid', type: 'c' },
        { label: 'Wonen', type: 'c' },
        { label: 'Dagbesteding', type: 'c' },
        { label: 'Vrije tijd', type: 'c' },
        { label: 'Financiën', type: 'c' },
        { label: 'Justitie', type: 'c' },
        { label: 'Veiligheid', type: 'c' },
      ]},
      { titel: 'Resultaten (1–6, herhaalbaar)', velden: [
        { label: 'Resultaat 1–6: omschrijving + wie helpt + verwacht begin/eind', type: 'g' },
        { label: 'Is er sprake van een ingekochte zorgaanbieder?', type: 'b' },
        { label: 'Afspraken rondom regie', type: 'c' },
      ]},
      { titel: 'Evaluatie (bij vervolgdeel PP)', velden: [
        { label: 'Zijn de resultaten uit het perspectiefplan behaald?', type: 'b' },
        { label: 'Kan de indicatie gesloten worden?', type: 'b' },
        { label: 'Per wanneer is de nieuwe situatie ontstaan?', type: 'g' },
        { label: 'Datum ontvangst Vervolgdeel PP', type: 'g' },
        { label: 'Evaluatie resultaat 1–7', type: 'b' },
        { label: 'Nieuwe resultaten 1–7 + wie ondersteunt + begin/eind', type: 'g' },
        { label: 'Afspraken voor de komende periode', type: 'c' },
        { label: 'Volgend evaluatiegesprek', type: 'g' },
      ]},
    ],
    vervolgpaden: [
      { label: '→ Besluitvorming', desc: 'Onderzoek afgerond, naar besluit', type: 'ok' },
      { label: '→ Perspectiefplan', desc: 'PP opstellen op basis van onderzoek', type: 'opt' },
      { label: '→ Archiveren', desc: 'Melding ingetrokken / afgesloten', type: 'back' },
      { label: '← Terug Melding', desc: 'Correctie nodig in stap Melding', type: 'back' },
    ]
  },
  {
    id: 'stap-30',
    num: 'Zaakstap 30',
    title: 'Perspectiefplan opstellen',
    sub: 'iJw § Toeleiden',
    color: '#1D9E75',
    fase: 'Perspectiefplan',
    preview: ['Startdatum indicatie','Zorgaanbieder · Segment · Product','Motivering PP'],
    secties: [
      { titel: 'Datapunten voor Monitoring', velden: [
        { label: 'Welk Perspectiefplan wil je opstellen? (initieel / vervolgdeel)', type: 'b' },
        { label: 'Gecontroleerd op lopende voorzieningen?', type: 'b' },
        { label: 'Gecontroleerd op stapeling?', type: 'b' },
        { label: 'Startdatum indicatie', type: 'g' },
        { label: 'Einddatum indicatie', type: 'g' },
        { label: 'Zorgaanbieder', type: 'c' },
        { label: 'Segment/Perceel', type: 'c' },
        { label: 'Product', type: 'c' },
        { label: 'Ondersteuningsprofiel/intensiteit', type: 'c' },
        { label: 'Volume/eenheid/frequentie', type: 'c' },
        { label: 'Vervoerscomponent groeps- / individueel vervoer', type: 'o' },
        { label: 'Motivering (initieel PP + vervolgdeel PP)', type: 'c' },
      ]},
    ],
    vervolgpaden: [
      { label: '→ Controle Kwaliteit', desc: 'PP ter beoordeling kwaliteitscheck', type: 'ok' },
    ]
  },
  {
    id: 'stap-40',
    num: 'Zaakstap 40',
    title: 'Controle Perspectiefplan door Kwaliteit',
    sub: 'Kwaliteitsborging',
    color: '#7F77DD',
    fase: 'Perspectiefplan',
    preview: ['PP inhoud akkoord?','Aanpassingen nodig?'],
    secties: [
      { titel: 'Datapunten voor Monitoring', velden: [
        { label: 'Is de inhoud van het Perspectiefplan akkoord?', type: 'b' },
        { label: 'Zijn er aanpassingen op het Perspectiefplan nodig?', type: 'b' },
      ]},
    ],
    vervolgpaden: [
      { label: '✓ PP akkoord → Versturen', desc: 'PP versturen naar inwoner', type: 'ok' },
      { label: '↩ Aanpassen PP', desc: 'Terug naar PP opstellen voor correctie', type: 'back' },
    ]
  },
  {
    id: 'stap-50',
    num: 'Zaakstap 50',
    title: 'Perspectiefplan versturen naar inwoner',
    sub: 'iJw § Toeleiden',
    color: '#1D9E75',
    fase: 'Perspectiefplan',
    preview: ['Datum PP verstuurd','Akkoord inwoner','Toestemming delen zorgaanbieder'],
    secties: [
      { titel: 'Datapunten voor Monitoring', velden: [
        { label: 'Perspectiefplan verstuurd op (datum)', type: 'g' },
        { label: 'Verstuurd naar', type: 'c' },
        { label: 'Is de inwoner akkoord met het Perspectiefplan?', type: 'b' },
        { label: 'Akkoord op Perspectiefplan ontvangen op', type: 'g' },
        { label: 'Toestemming voor het delen van het PP met de zorgaanbieder', type: 'b' },
        { label: 'Wordt er een zorgaanbieder ingezet?', type: 'b' },
        { label: 'Is er (mede) sprake van PGB?', type: 'b' },
        { label: 'PGB-plan ontvangen op', type: 'g' },
        { label: 'Aanpassingen definitief PP overgenomen in Melding & Onderzoek?', type: 'b' },
        { label: 'Wil je een nieuwe versie van PP versturen?', type: 'b' },
      ]},
    ],
    vervolgpaden: [
      { label: '→ Besluitvorming', desc: 'Naar besluitvorming in te zetten zorg', type: 'ok' },
    ]
  },
  {
    id: 'stap-60',
    num: 'Zaakstap 60',
    title: 'Besluitvorming in te zetten zorg',
    sub: 'iJw § Besluit',
    color: '#F26722',
    fase: 'Besluitvorming',
    preview: ['Besluit toekenning/afwijzing/intrekking','Leveringsvorm ZIN/PGB','Startdatum per segment'],
    secties: [
      { titel: 'Besluit & Checks', velden: [
        { label: 'Besluit (toekenning / afwijzing / intrekking / anders)', type: 'b' },
        { label: 'Wettelijke vertegenwoordigers vastgelegd in MC?', type: 'b' },
        { label: 'Gecontroleerd op lopende voorzieningen?', type: 'b' },
        { label: 'Gecontroleerd op stapeling?', type: 'b' },
        { label: 'Is er medisch advies ingewonnen?', type: 'b' },
        { label: 'Leveringsvorm (ZIN / PGB / eigen netwerk)', type: 'b' },
        { label: 'Type vertegenwoordiger · Wettelijke vertegenwoordiger', type: 'c' },
        { label: 'Wie levert de zorg?', type: 'c' },
        { label: 'Zorgplan ontvangen op', type: 'g' },
        { label: 'Beoordelingsformulier kwaliteit zorgaanbieder ontvangen op', type: 'g' },
      ]},
      { titel: 'Toekenning ZIN – per Segment (1–5, herhaalbaar)', velden: [
        { label: 'Segment / aanvullende producten', type: 'c' },
        { label: 'Ondersteuningsprofiel · Perceel', type: 'c' },
        { label: 'Omschrijving product', type: 'c' },
        { label: 'Intensiteit · Volume/eenheid/frequentie', type: 'c' },
        { label: 'Zorgaanbieder (per segment)', type: 'c' },
        { label: 'Startdatum 1–5', type: 'g' },
        { label: 'Einddatum 1–5', type: 'g' },
      ]},
      { titel: 'PGB', velden: [
        { label: 'Berekeningsformulier PGB toegevoegd aan documenten?', type: 'b' },
        { label: 'Startdatum PGB', type: 'g' },
        { label: 'Einddatum PGB', type: 'g' },
        { label: 'Volume/eenheid/frequentie PGB', type: 'c' },
        { label: 'Omschrijving ondersteuning ouders/netwerk', type: 'c' },
        { label: 'Reden afsluiten incidentele maatwerkovereenkomst', type: 'c' },
      ]},
    ],
    vervolgpaden: [
      { label: '✓ Toekenning → Bestelling', desc: 'Naar Bestelling plaatsen', type: 'ok' },
      { label: '✗ Afwijzing → Beschikking', desc: 'Afwijzing vastleggen', type: 'no' },
      { label: '⊘ Intrekking → Archiveren', desc: 'Intrekking verwerken', type: 'back' },
      { label: '⟳ Maatwerkovereenkomst', desc: 'Naar aanvraag maatwerkovereenkomst', type: 'opt' },
    ]
  },
  {
    id: 'stap-70',
    num: 'Zaakstap 70',
    title: 'Aanvraag maatwerkovereenkomst',
    sub: 'Optioneel pad',
    color: '#94A3B8',
    fase: 'Optioneel pad',
    preview: ['KVK uittreksel','Productcode + tarief','Begin-/Einddatum overeenkomst'],
    secties: [
      { titel: 'Aanvraag gegevens', velden: [
        { label: 'Aanbieder voldoet aan voorwaarden?', type: 'b' },
        { label: 'KVK uittreksel toegevoegd?', type: 'b' },
        { label: 'Begindatum overeenkomst', type: 'g' },
        { label: 'Einddatum overeenkomst', type: 'g' },
        { label: 'Datum verwijzing', type: 'g' },
        { label: 'Datum start zorg', type: 'g' },
        { label: 'Einddatum zorg', type: 'g' },
        { label: 'Productcode + omschrijving + tarief + aantal + eenheid + frequentie (1–3)', type: 'c' },
      ]},
    ],
    vervolgpaden: [
      { label: '→ Controle & Invoer PDC', desc: 'Naar stap 80/90 voor verwerking', type: 'ok' },
    ]
  },
  {
    id: 'stap-80',
    num: 'Zaakstap 80/90',
    title: 'Controle & Invoer PDC',
    sub: 'Versturen, accorderen en verwerken',
    color: '#94A3B8',
    fase: 'Optioneel pad',
    preview: ['Gegevens compleet?','Datum retour ontvangst','Ingevoerd in PDC?'],
    secties: [
      { titel: 'Controle & Verwerking', velden: [
        { label: 'Gegevens compleet en correct?', type: 'b' },
        { label: 'Datum verstuurd naar aanbieder', type: 'g' },
        { label: 'Datum retour ontvangst', type: 'g' },
        { label: 'Maatwerkovereenkomst akkoord en ondertekend?', type: 'b' },
        { label: 'Gegevens ingevoerd in PDC?', type: 'b' },
      ]},
    ],
    vervolgpaden: [
      { label: '→ Bestelling plaatsen', desc: 'Einde optioneel pad, terug naar hoofdflow', type: 'ok' },
    ]
  },
  {
    id: 'stap-100',
    ...
    sub: 'iJw § Leveren',
    color: '#005496',
    fase: 'Bestelling & Beschikking',
    preview: ['Verlenging duurzaam?','Bestelling in productshop','Nieuwe einddatum arrangement'],
    secties: [
      { titel: 'Datapunten voor Monitoring', velden: [
        { label: 'Is er sprake van Verlenging duurzaam?', type: 'b' },
        { label: 'Voldoet de Verlenging duurzaam aan de voorwaarden?', type: 'b' },
        { label: 'Eerste dag van verlenging arrangement', type: 'g' },
        { label: 'Nieuwe einddatum arrangement', type: 'g' },
        { label: 'Voorziening / PGB besteld in productshop?', type: 'b' },
        { label: 'Bestelling komt overeen met gegevens Besluitvorming?', type: 'b' },
        { label: 'Einddatum voorziening aangepast?', type: 'b' },
        { label: 'Naar welk adres moet de beschikking gestuurd worden?', type: 'c' },
        { label: 'Betreft het crisis?', type: 'b' },
        { label: 'Is er (mede) sprake van PGB?', type: 'b' },
      ]},
    ],
    vervolgpaden: [
      { label: '→ Bestelling controleren', desc: 'Backoffice controle stap 120', type: 'ok' },
      { label: '→ PGB: Berekening controleren', desc: 'Optioneel, alleen bij PGB (stap 110)', type: 'opt' },
    ]
  },
  {
    id: 'stap-110',
    num: 'Zaakstap 110',
    title: 'PGB: Berekening controleren',
    sub: 'Optioneel – alleen bij PGB',
    color: '#94A3B8',
    fase: 'Optioneel pad',
    preview: ['Berekeningsformulier juist?','Bestelling juist ingevuld?'],
    secties: [
      { titel: 'PGB Controle', velden: [
        { label: 'Berekeningsformulier en bestelling juist ingevuld?', type: 'b' },
      ]},
    ],
    vervolgpaden: [
      { label: '→ Bestelling controleren', desc: 'Terug naar stap 120', type: 'ok' },
    ]
  },
  {
    id: 'stap-120',
    ...
    sub: 'Backoffice controle',
    color: '#005496',
    fase: 'Bestelling & Beschikking',
    preview: ['Is de bestelling akkoord?','Correctie nodig door wie?'],
    secties: [
      { titel: 'Controle', velden: [
        { label: 'Is de bestelling akkoord?', type: 'b' },
        { label: 'Correctie nodig door (wie)?', type: 'c' },
      ]},
    ],
    vervolgpaden: [
      { label: '→ Opstellen beschikking', desc: 'Naar stap 130', type: 'ok' },
    ]
  },
  {
    id: 'stap-130',
    num: 'Zaakstap 130',
    title: 'Opstellen beschikking',
    sub: 'iJw § Leveren',
    color: '#005496',
    fase: 'Bestelling & Beschikking',
    preview: ['Beschikking verzonden?','Datum verzending','Via post / e-mail / DigiD'],
    secties: [
      { titel: 'Datapunten voor Monitoring', velden: [
        { label: 'Is de beschikking verzonden?', type: 'b' },
        { label: 'Beschikking verzonden op (datum)', type: 'g' },
        { label: 'Verstuurd via (post / e-mail / DigiD)', type: 'c' },
        { label: 'E-mailadres gecontroleerd?', type: 'b' },
        { label: 'Is er alsnog een correctie van de bestelling nodig?', type: 'b' },
        { label: 'Is er sprake van ASH zonder crisis?', type: 'b' },
        { label: 'Is er (mede) sprake van PGB?', type: 'b' },
      ]},
    ],
    vervolgpaden: [
      { label: '→ PGB: Controle verwerking PDC', desc: 'Optioneel – stap 140 bij PGB', type: 'opt' },
      { label: '→ Regie en Monitoren', desc: 'Naar stap 150', type: 'ok' },
    ]
  },
  {
    id: 'stap-140',
    num: 'Zaakstap 140',
    title: 'PGB: Controle verwerking in PDC',
    sub: 'Verificatie PGB-verwerking en SVB-melding',
    color: '#94A3B8',
    fase: 'Optioneel pad',
    preview: ['PGB correct verwerkt?','Datum melding PGB bij SVB'],
    secties: [
      { titel: 'PGB Verwerking', velden: [
        { label: 'PGB correct verwerkt in PDC?', type: 'b' },
        { label: 'Datum melding PGB bij SVB', type: 'g' },
        { label: 'Zaak doorzetten naar Regie en Monitoren?', type: 'b' },
      ]},
    ],
    vervolgpaden: [
      { label: '→ Regie en Monitoren', desc: 'Einde optioneel pad, naar stap 150', type: 'ok' },
    ]
  },
  {
    id: 'stap-150',
    num: 'Zaakstap 150',
    title: 'Regie en Monitoren',
    sub: 'iJw § Regie',
    color: '#1D9E75',
    fase: 'Regie & Afsluiting',
    preview: ['Datum evaluatie','Startdatum huidige toewijzing','Huidige zorgaanbieder','Evaluatieverslag'],
    secties: [
      { titel: 'Datapunten voor Monitoring', velden: [
        { label: 'Wat wil je vastleggen?', type: 'c' },
        { label: 'Datum evaluatie', type: 'g' },
        { label: 'Startdatum huidige toewijzing', type: 'g' },
        { label: 'Is er een onafhankelijke cliëntondersteuner aanwezig?', type: 'b' },
        { label: 'Wat is het vervolg? (regie / besluitvorming / archiveren)', type: 'b' },
        { label: 'Huidige indicatie en duur', type: 'c' },
        { label: 'Huidige zorgaanbieder', type: 'c' },
        { label: 'Evaluatieverslag', type: 'c' },
        { label: 'Aanwezigen gesprek', type: 'c' },
      ]},
    ],
    vervolgpaden: [
      { label: '↩ Regie voortzetten', desc: 'Monitoring loopt door', type: 'ok' },
      { label: '→ Besluitvorming', desc: 'Nieuw besluit nodig', type: 'opt' },
      { label: '→ Archiveren', desc: 'Zorg afgerond', type: 'back' },
    ]
  },
  {
    id: 'stap-160',
    num: 'Zaakstap 160',
    title: 'Archiveren',
    sub: 'Afsluiting dossier',
    color: '#4A6180',
    fase: 'Regie & Afsluiting',
    preview: ['Beschikkingen aangevinkt?','Datum sluiting zaak','Doorlooptijd'],
    secties: [
      { titel: 'Afsluiting', velden: [
        { label: 'Beschikkingen, PP, contactmomenten aangevinkt?', type: 'b' },
        { label: 'Kan de zaak afgesloten worden?', type: 'b' },
        { label: 'Datum sluiting zaak', type: 'g' },
        { label: 'Doorlooptijd (datum melding → datum sluiting)', type: 'g' },
      ]},
    ],
    vervolgpaden: [
      { label: '→ Toetsing Kwaliteit', desc: 'Steekproef kwaliteitstoetsing stap 170', type: 'opt' },
      { label: '→ Sluit Zaak', desc: 'Naar definitieve afsluiting stap 221', type: 'ok' },
    ]
  },
  {
    id: 'stap-170',
    num: 'Zaakstap 170',
    title: 'Toetsing Kwaliteit (Jeugd)',
    sub: 'Steekproef kwaliteitstoetsing',
    color: '#7F77DD',
    fase: 'Regie & Afsluiting',
    preview: ['Wordt deze zaak getoetst?','Toetsplan ingevuld?'],
    secties: [
      { titel: 'Kwaliteitstoetsing', velden: [
        { label: 'Wordt deze zaak getoetst?', type: 'b' },
        { label: 'Is het Toetsplan ingevuld en toegevoegd?', type: 'b' },
      ]},
    ],
    vervolgpaden: [
      { label: '→ Sluit Zaak', desc: 'Naar definitieve afsluiting stap 221', type: 'ok' },
  ...
  },
  {
    ...
    num: 'Zaakstap 221',
    title: 'Sluit Zaak',
    sub: 'Definitieve afsluiting in het systeem',
    color: '#002E56',
    fase: 'Regie & Afsluiting',
    preview: ['Afsluitreden MC','Zaakresultaat'],
    secties: [
      { titel: 'Afsluiting', velden: [
        { label: 'Afsluitreden MC', type: 'b' },
        { label: 'Zaakresultaat', type: 'b' },
      ]},
    ],
    vervolgpaden: [
      { label: '✓ Zaak gesloten', desc: 'Definitief afgesloten', type: 'ok' },
    ]
  },
];

/* Fase-groepen met kleur voor de flow */
const JEUGD_FASEN = [
  { naam: 'Melding & Onderzoek', kleur: '#005496', stappen: ['stap-10','stap-20'] },
  { naam: 'Perspectiefplan', kleur: '#1D9E75', stappen: ['stap-30','stap-40','stap-50'] },
  { naam: 'Besluitvorming', kleur: '#F26722', stappen: ['stap-60'] },
  { naam: 'Optioneel pad', kleur: '#94A3B8', stappen: ['stap-70','stap-80'] },
  { naam: 'Bestelling & Beschikking', kleur: '#005496', stappen: ['stap-100','stap-110','stap-120','stap-130','stap-140'] },
  { naam: 'Regie & Afsluiting', kleur: '#1D9E75', stappen: ['stap-150','stap-160','stap-170','stap-221'] },
];

/* ─── Open/Close Jeugd schema view ─── */
function openJeugdSchema() {
  document.getElementById('proc-library').style.display = 'none';
  const sv = document.getElementById('jeugd-schema-view');
  sv.style.display = 'flex';
  renderJeugdFlow();
}

function closeJeugdSchema() {
  document.getElementById('jeugd-schema-view').style.display = 'none';
  document.getElementById('proc-library').style.display = 'block';
}

/* ─── Render the flow ─── */
function renderJeugdFlow() {
  const container = document.getElementById('jeugd-flow');
  if (!container) return;

  const tagClassMap = { g: 'jf-tg', b: 'jf-tb', o: 'jf-to', c: 'jf-tc' };

  let html = '';
  let firstFase = true;

  JEUGD_FASEN.forEach(fase => {
    const stappen = fase.stappen.map(id => JEUGD_ZAAKSTAPPEN.find(s => s.id === id)).filter(Boolean);
    if (!stappen.length) return;

    // connector between fasen
    if (!firstFase) html += `<div class="jf-conn"><div class="jf-arr"></div></div>`;
    firstFase = false;

    // For optioneel pad, show as side-branch style but still inline
    const isOpt = fase.naam === 'Optioneel pad';

    stappen.forEach((stap, si) => {
      if (si > 0) html += `<div class="jf-conn"><div class="jf-arr"></div></div>`;

      const previewTags = (stap.preview || []).map(p =>
        `<span class="jf-tag jf-tg">${escHtml(p)}</span>`
      ).join('');

      html += `
      <div class="jf-fase${isOpt ? ' jf-fase-opt' : ''}" onclick="openZaakstapModal('${stap.id}')" style="border-left:4px solid ${fase.kleur};">
        <div class="jf-fase-tag" style="background:${fase.kleur};">${escHtml(fase.naam)}</div>
        <div class="jf-fase-body">
          <div class="jf-stap" style="background:${fase.kleur};">
            <div class="jf-stap-left">
              <span class="jf-num">${escHtml(stap.num)}</span>
              <div>
                <div class="jf-title">${escHtml(stap.title)}</div>
                <div class="jf-sub">${escHtml(stap.sub)}</div>
              </div>
            </div>
            <div style="display:flex;align-items:center;gap:8px;">
              <span class="jf-click-hint">klik voor details →</span>
              <span class="jf-badge">${stap.vervolgpaden.length} paden</span>
            </div>
          </div>
          <div class="jf-data">
            <div class="jf-data-title">Kerngegevens</div>
            <div class="jf-tags">${previewTags}</div>
          </div>
        </div>
      </div>`;
    });
  ...
  // End pill
  html += `
  <div class="jf-conn"><div class="jf-arr"></div></div>
  <div class="jf-einde"><div class="jf-einde-pill">Zaak gesloten</div></div>`;

  container.innerHTML = html;
}

/* ─── Open zaakstap modal ─── */
function openZaakstapModal(stapId) {
  const stap = JEUGD_ZAAKSTAPPEN.find(s => s.id === stapId);
  if (!stap) return;

  const overlay = document.getElementById('zaakstap-overlay');
  const header = document.getElementById('zs-modal-header');
  const numEl = document.getElementById('zs-modal-num');
  const titleEl = document.getElementById('zs-modal-title');
  const subEl = document.getElementById('zs-modal-sub');
  const bodyEl = document.getElementById('zs-modal-body');

  // Set header colour
  header.style.background = stap.color;
  numEl.textContent = stap.num;
  titleEl.textContent = stap.title;
  subEl.textContent = stap.sub;

  const tagClassMap = { g: 'zs-tg', b: 'zs-tb', o: 'zs-to', c: 'zs-tc' };
  const pathClassMap = { ok: 'zs-path-ok', no: 'zs-path-no', opt: 'zs-path-opt', back: 'zs-path-back' };

  let bodyHtml = '';

  // Secties
  stap.secties.forEach(sectie => {
    const tags = sectie.velden.map(v =>
      `<span class="zs-tag ${tagClassMap[v.type] || 'zs-tc'}">${escHtml(v.label)}</span>`
    ).join('');
    bodyHtml += `
    <div class="zs-section">
      <div class="zs-section-title">${escHtml(sectie.titel)}</div>
      <div class="zs-tags">${tags}</div>
    </div>`;
  });

  // Vervolgpaden
  if (stap.vervolgpaden && stap.vervolgpaden.length) {
    const paths = stap.vervolgpaden.map(p =>
      `<div class="zs-path ${pathClassMap[p.type] || 'zs-path-back'}">
        <div style="font-weight:700;font-size:.72rem;">${escHtml(p.label)}</div>
        <div style="font-size:.65rem;opacity:.8;margin-top:1px;">${escHtml(p.desc)}</div>
      </div>`
    ).join('');
    bodyHtml += `
    <div class="zs-section" style="background:var(--lg);">
      <div class="zs-section-title">Vervolgpad na deze stap</div>
      <div style="display:flex;flex-wrap:wrap;gap:6px;">${paths}</div>
    </div>`;
  }
...
/* ─── Close modal ─── */
function closeZaakstapModal(e) {
  if (e && e.target !== document.getElementById('zaakstap-overlay')) return;
  document.getElementById('zaakstap-overlay').style.display = 'none';
