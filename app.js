...
function renderProcLibrary() {
  ...
  grid.innerHTML = visProcs.map(p => {
    ...
    const isFlow = p.type === 'flow' || (typeof FASE_PROC_IDS !== 'undefined' && FASE_PROC_IDS.indexOf(p.id) >= 0);
    const typeBadge = `<div class="proc-type-badge ${isFlow ? 'proc-type-flow-badge' : 'proc-type-schema-badge'}">${isFlow ? '\u2261 Stapsgewijs' : '\u29c6 Schema'}</div>`;
    return `<div class="proc-card${currentProcId === p.id ? ' active-card' : ''}${isFlow ? ' proc-card-flow' : ''}" onclick="openProcess('${p.id}')">
      ${typeBadge}
...
function rebuildProcCatDropdowns() {
  ...
  if (e) { const prev = e.value; e.innerHTML = '<option value="">— categorie —</option>' + opts; e.value = prev; }
  const fc = document.getElementById('fase-cat-select');
  if (fc) { const prev = fc.value; fc.innerHTML = '<option value="">— categorie —</option>' + opts; fc.value = prev; }
}
...
/* ── renderProcLibrary: type badges geïntegreerd ── */
